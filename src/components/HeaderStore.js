...
function getTopTags(BlogData, limit = 10) {
  const tagCounts = {};
  console.log("HeaderStore tagCounts",tagCounts);
  BlogData.forEach((blog) => {
    const tags = blog.tag && blog.tag.length ? blog.tag[0].tags : [];
    console.log("HeaderStore tags",tags);
    tags.forEach((tag) => {
  ...
  const sortedTags = Object.entries(tagCounts)
    .sort((a, b) => b[1] - a[1])
    .map((entry) => entry[0]);
    console.log("HeaderStore sortedTags",sortedTags);
...
const HeaderStore = {
  ...
  after_render: async () => {
    ...
    const HeaderStoreMid = document.querySelector(".nav-mid");
    ...
    window.addEventListener("scroll", () => {
      ...
      if (scrollHeight > navHeight) {
        HeaderStore.classList.add("fix");
        HeaderStoreMid.classList.add("hide");
        utilityLogo.classList.add("show");
      } else {
        HeaderStore.classList.remove("fix");
        HeaderStoreMid.classList.remove("hide");
