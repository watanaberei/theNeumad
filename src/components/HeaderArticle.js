...
function getTopTags(BlogData, limit = 10) {
  const tagCounts = {};
  console.log("HeaderArticle tagCounts",tagCounts);
  BlogData.forEach((blog) => {
    const tags = blog.tag && blog.tag.length ? blog.tag[0].tags : [];
    console.log("HeaderArticle tags",tags);
    tags.forEach((tag) => {
  ...
  const sortedTags = Object.entries(tagCounts)
    .sort((a, b) => b[1] - a[1])
    .map((entry) => entry[0]);
    console.log("HeaderArticle sortedTags",sortedTags);
...
const HeaderArticle = {
  ...
  after_render: async () => {
    ...
    const HeaderArticle = document.querySelector(".header");
    const HeaderArticleMid = document.querySelector(".nav-mid");
    ...
    window.addEventListener("scroll", () => {
      ...
      if (scrollHeight > navHeight) {
        HeaderArticle.classList.add("fix");
        HeaderArticleMid.classList.add("hide");
        utilityLogo.classList.add("show");
      } else {
        HeaderArticle.classList.remove("fix");
        HeaderArticleMid.classList.remove("hide");
