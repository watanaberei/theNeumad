...
function storeSelectedLocation(result) {
  if (result && result.result && result.result.geometry) {
    localStorage.setItem('selectedLocation', JSON.stringify(result.result.geometry.coordinates));
  }
}
const HeaderMap = {
  render: () => {
    const tags = ['Tag 1', 'Tag 2', 'Tag 3']; // Replace this with the actual tags you want to display
    const tagsHTML = allTags(tags);
