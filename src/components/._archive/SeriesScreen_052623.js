...
const SeriesScreen = {
  render: async () => {
    ...
    ${articleNeumadsTrails.map((articleNeumadsTrail) => `${ArticleNeumadsTrail.render(articleNeumadsTrail)}`).join("\n")}
  ...
  after_render: () => {
    ...
    async function loadData() {
      const articleNeumadsTrails = await getArticleNeumadsTrails(3, 3 * ++index + 1);
