...
function getTopTags(BlogData, limit = 10) {
  ...
  const HeaderMap = {
    ...
    render: async () => {
        ...
        HeaderMap.BlogData = [...articleData, ...storeData, ...postData];
        const topTags = getTopTags(HeaderMap.BlogData, 10);
        ...
        <nav class="navigation container nav-top">
            ...
            <!-- DATA -->
            <section class="data">
                <div class="blog-data" id="blog-data">
                ${tagsHTML}
                <div class="metadata-tag" id="more-tag">
                    <span class="metadata-tag-text text01">More</span>
                </div>
                </div>
            </section>
            <!-- DATA -->
    ...
    after_render: async () => {
      setTimeout(() => {
        ...
        moreTag.addEventListener('click', HeaderMap.handleMoreClick);
        const tagElements = document.querySelectorAll('.metadata-tag');
        tagElements.forEach((tagElement) => {
          tagElement.addEventListener('click', HeaderMap.handleTagClick);
        });
      }, 0);
    },
  ...
  handleTagClick: (event) => {
    ...
    const filteredPosts = tagsFilter(HeaderMap.BlogData, tag);
        ...
        // const filteredPosts = this.BlogData.filter(
        //     (blog) => blog.tags && blog.tags[0].tags.includes(tags)
        //   );
          // Update UI
        ...
        // const tag = event.target.textContent;
        // const filteredPosts = this.BlogData.filter(blog => blog.tag[0].tags.includes(tag));
        // Now, filteredPosts contains the posts that have the clicked tag
        // You can update the UI to display these posts
        // For example, if you have a container for the posts, you can do:
        // const postsContainer = document.getElementById('blog-layout');
        // postsContainer.innerHTML = filteredPosts.map(post => AllBlog.render(post)).join('\n');
