...
export default function NonFeaturedBlog(nonFeaturedBlog) {
  const sentence = nonFeaturedBlog.tag || "";
  const tagElements = generateTags(sentence);

  // Check if the thumbnail exists and has a valid URL
  const thumbnailUrl = nonFeaturedBlog.thumbnail && nonFeaturedBlog.thumbnail.url ? nonFeaturedBlog.thumbnail.url : "";

  const title = nonFeaturedBlog.title ? nonFeaturedBlog.title : "";
  const truncatedTitle = title.length > 90 ? title.substr(0, 90) + " ..." : title;
