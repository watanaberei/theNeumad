...
export function initNearbyMap() {
  ...
  const nearbyMap = new nearbyMapboxgl.Map({
  ...
  nearbyMap.on("load", function () {
    ...
    // nearbyMap.setPaintProperty('building', 'fill-opacity', [
    //   'interpolate',
    //   // Set the exponential rate of change to 0.5
    //   ['exponential', 0.5],
    //   ['zoom'],
    ...
    // -----------------BOUNDARIES----------------- //
    // CITY
    // nearbyMap.addSource("city-boundaries", {
    //   type: "geojson",
    //   data: "https://raw.githubusercontent.com/uber-web/kepler.gl-data/master/county_unemployment/data.geojson",
    // });
    ...
    // 3D building
    // Insert the layer beneath any symbol layer.
    const layers = nearbyMap.getStyle().layers;
    const labelLayerId = layers.find(
      (layer) => layer.type === "symbol" && layer.layout["text-field"]
    ).id;
...
//   const layer = nearbyMap
// .getStyle()
// .layers.find((layer) => layer.id === 'natural-point-label');
// // Add in a pitch condition to the existing layers filter
// // so that it only renders when the pitch is low
// const lowPitchFilter = ['all', layer.filter, ['<', ['pitch'], 60]];
// nearbyMap.setFilter('natural-point-label', lowPitchFilter);
...
// // Add in a new layer with the updated styling
// nearbyMap.addLayer(layer, 'natural-point-label');
  // MUseums
  // -----------------BUILDINGS----------------- //
  ...
  //   const rotation = initialBearing + animationTime * 2.0;
  //   nearbyMap.setBearing(rotation % 360);
  ...
  if (navigator.geolocation) {
    navigator.geolocation.getCurrentPosition((position) => {
      ...
      nearbyMap.setCenter(userCoordinates);
    });
  }
