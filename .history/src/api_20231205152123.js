  ...
  try {
    ...
    const data = stores.map((stores) => {
      return {
        ...
        nearbyStore: {
          headline: stores?.nearbyStoresCollection?.headline,
          hours: store?.nearbyStoresCollection?.hours,
          logo: store?.media?.logo?.url,
          location: store?.locationCollection?.items.map((item) => ({
            type: item?.type,
            geolocation: {
