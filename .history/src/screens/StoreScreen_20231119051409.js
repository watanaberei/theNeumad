...
const StoreScreen = {
  render: async () => {
    ...
    const store = await dataBlog(); // Adjust this call according to your API fetching method

    // const store = storeData.filter(data => data.variant === 'store');
    console.log("store", store)
    console.log("featured", store.featured);

    
    // Render store data
    const storeHtml = store.map(store => {
      ...
      const headlineHtml = documentToHtmlString(store.headline.text);
