...
// GALLERY IMG
    ...
    // Render store data
    const storeHtml = stores.map(stores => `
      <div class="store">
      <span class="header06">
      ${stores.title} ${stores.headline}
      <span>
        <span class="header06">
            ${stores.headline.text}
        </span>
        <span class="header06">${stores.description}</span
        <img src="${stores.galleryHTML.url}" alt="" /> 
        ${stores.galleryHTML.url}
