...
//   let storeDetails; // This will hold the store details accessible by both render and after_render
  ...
  const StoreScreen = {
    store: null, // Declaring `store` as a property of StoreScreen
      render: async () => {
    ...
    // // GRID //
        ...
        after_render: () => {
            ...
            // Use this.store instead of global store
            if (!this.store || !this.store.location || !this.store.location.geolocation) {
                console.error('Store details not found or does not contain location data.');
                return;
            }
    
            const { lat, lon } = this.store.location.geolocation;
