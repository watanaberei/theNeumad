  ...
  const StoreScreen = {
      render: async () => {
       ...
       // Store
        ...
        const nearbyLogoData = nearbyLogo && Array.isArray(nearbyLogo) && nearbyLogo.length ? nearbyLogo : [];
...
function generateLogoCarouselHTML(nearbyLogo) {
    ...
    nearbyLogo.slice(0, 3).forEach(nearbyLogoItem => {
        nearbyLogoHTML += `
                <img src="${nearbyLogoItem.url}" class="galleryItem" alt="" />
      `;
    });
    console.log("mediaGallery",nearbyLogo);
    return nearbyLogoyHTML;
}
