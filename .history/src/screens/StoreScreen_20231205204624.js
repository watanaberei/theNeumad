...
const renderOptions = {
  ...
  let storeDetails; // This will hold the store details accessible by both render and after_render

  const StoreScreen = {
      render: async () => {
        const request = parseRequestUrl();
        console.log("Request slug:", request.slug);
        storeDetails = await dataBlog.getData(); // Assuming this contains the store details
        ...
        // Summary
        const limitedBest03 = store.summary.best && store.summary.best.length ? store.summary.best.slice(0, 3) : [];
    ...
    // // GRID //
        ...
        after_render: () => {
            ...
            // Here you use the passed `storeData` instead of `store`
            const storeLon = storeData.location.geolocation.lon;
            const storeLat = storeData.location.geolocation.lat;


            if (!storeDetails || !storeDetails.location) {
                console.error('Store details not found or does not contain location data.');
                return;
            }
    
            const { lat, lon } = storeDetails.location.geolocation; // Adjust this according to your actual data structure
    
            const storeLocation = [lon, lat]; // The order is [longitude, latitude]
            
            // Assuming you have access to a 'store' object with the location data
            // const storeLocation = [storeLon, storeLat]; // The order is [longitude, latitude]
            
            // Initialize the map object with the store's location as the center
            const map = new mapboxgl.Map({
                container: 'map-container', // container ID
                style: 'mapbox://styles/mapbox/streets-v11', // style URL
                center: storeLocation, // starting position [lng, lat]
            ...
            // Assuming you have access to the user's current location
            // This should be replaced with actual user location data
            // const userLocation = [/* user longitude */, /* user latitude */];
            ...
            // Fit the map to show both the store and the user's location
