...
function renderFeatures(features, map) {
  ...
  features.forEach((store) => {
    ...
    // Render Reviews
    const storeListing = createGeojsonStoreListing(store, onClick);
    const reviewListing = createGeojsonReviewListing(store, onClick);
    const articleListing = createGeojsonArticleListing(store, onClick);
    const blogListing = createGeojsonBlogListing(store, onClick);
    
    

    if (listing && listing.innerHTML.trim() !== "") {
      document.getElementById("postListing").appendChild(listing);
  }

  if (storeListing && storeListing.innerHTML.trim() !== "") {
      document.getElementById("postStore").appendChild(storeListing);
  }

  if (reviewListing && reviewListing.innerHTML.trim() !== "") {
      document.getElementById("postReview").appendChild(reviewListing);
  }

  if (articleListing && articleListing.innerHTML.trim() !== "") {
      document.getElementById("postArticle").appendChild(articleListing);
  }

  if (blogListing && blogListing.innerHTML.trim() !== "") {
      document.getElementById("postBlog").appendChild(blogListing);
  }
  });
}
