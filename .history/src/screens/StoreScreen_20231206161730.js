...
const StoreScreen = {
    render: async () => {
        // Replace with your existing logic to fetch store data
        const request = parseRequestUrl();
        const storeDetails = await dataBlog.getData();
        currentStore = storeDetails.find(store => store.slug === request.slug);
        ...
        this.store = storeDetails.find(store => store.slug === request.slug);
    ...
    // // GRID //
        ...
        `;
        return `<div>Your HTML content here</div>`; // Your HTML return code
    },
    ...
    // Check if currentStore data is available
    if (!currentStore || !currentStore.location || !currentStore.location.geolocation) {
        console.error('Store details not found or does not contain location data.');
        return;
    }

    // Extract latitude and longitude from currentStore
    const { lat, lon } = currentStore.location.geolocation;
    ...
    const map = new mapboxgl.Map({
        ...
        zoom: 13 // starting zoom
            ...
            // Example user location - this should be obtained from a user's actual location
