  ...
  try {
    ...
    const data = stores.map((stores) => {
      return {
        ...
        nearbyStore: {
          headline: stores?.nearbyStoreCollection?.headline,
          hours: stores?.nearbyStoreCollection?.hours,
          logo: stores?.nearbyStoreCollection?.media?.logo?.url,
          location: stores?.nearbyStoreCollection?.locationCollection?.items.map((item) => ({
            type: item?.type,
            geolocation: {
